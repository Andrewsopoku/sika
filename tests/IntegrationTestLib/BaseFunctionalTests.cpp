...
#ifdef _WIN32
const std::string DAEMON_FILENAME = "sikabitd.exe";
#else
const std::string DAEMON_FILENAME = "sikabitd";
#endif
...
void BaseFunctionalTests::startNode(size_t index) {
  ...
  config
    ...
    << "log-file=test_sikabitd_" << index << ".log" << std::endl;
...
#if defined WIN32
  std::string commandLine = "start /MIN \"sikabitd" + std::to_string(index) + "\" \"" + daemonPath.string() +
  ...
  if (pid == 0) {
    ...
    if (execl(pathToDaemon.c_str(), "sikabitd", "--testnet", dataDir.c_str(), "--config-file=daemon.conf", NULL) == -1) {
